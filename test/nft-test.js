...
describe("MyNFT", function () {
  ...
  it("should mint one NFT to sender", async function () {
    ...
    const tx = await myNFT.connect(acc1).createNFT(tokenURI);
    await tx.wait();
  ...
  it("should set the correct tokenURI", async function () {
    ...
    const tx1 = await myNFT.connect(owner).createNFT(tokenURI_1);
    await tx1.wait();
    const tx2 = await myNFT.connect(owner).createNFT(tokenURI_2);
    await tx2.wait();
